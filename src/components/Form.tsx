...
import {
  AdditionalStyle,
  getThemedSelectStyle,
  SelectVariant,
} from '@/utils/style';

import Label from './Label';

const ValueContainerSortBy: React.FC<
  ...
  return (
    components.ValueContainer && (
      <components.ValueContainer {...props}>
        {!!children && (
          <SortAlphaDownAlt className='absolute left-3 opacity-80' />
        )}
        {children}
      </components.ValueContainer>
    )
...
const TextArea: React.FC<TextareaHTMLAttributes<unknown>> = ({
  className,
  ...props
}) => {
  return (
...
const Checkbox: React.FC<InputHTMLAttributes<unknown>> = ({
  children,
  ...props
}) => {
...
const DatePickerComponent: React.FC<ReactDatePickerProps> = ({
  className,
  showTimeSelect,
  ...props
}) => {
...
const SelectProvince: React.FC<
  Partial<Async<OptionTypeBase>> & Props<OptionTypeBase, false>
> = (props) => {
  ...
  return (
    <Select
      ...
      loadOptions={async (val) => {
        ...
        return data.provinces.data.map(({ id, name }) => ({
          value: id,
          label: name,
        }));
      }}
...
const SelectCity: React.FC<
  Partial<Async<OptionTypeBase>> &
    Props<OptionTypeBase, false> & { provinceId: string }
> = (props) => {
  ...
  return (
    <Select
      ...
      loadOptions={async (val) => {
        ...
        return data.cities.data.map(({ id, name, type }) => ({
          value: id,
          label: `${type} ${name}`,
        }));
      }}
...
const SelectSubdistrict: React.FC<
  Partial<Async<OptionTypeBase>> &
    Props<OptionTypeBase, false> & { cityId: string }
> = (props) => {
  ...
  return (
    <Select
      ...
      loadOptions={async (val) => {
        ...
        return data.subdistricts.data.map(({ id, name }) => ({
          value: id,
          label: name,
        }));
      }}
...
const SelectVillage: React.FC<
  Partial<Async<OptionTypeBase>> &
    Props<OptionTypeBase, false> & { subdistrictId: string }
> = (props) => {
  ...
  return (
    <Select
      ...
      loadOptions={async (val) => {
        ...
        return data.villages.data.map(({ id, name }) => ({
          value: id,
          label: name,
        }));
      }}
...
const SelectRole: React.FC<ThemedSelectProps> = (props) => {
  ...
  const options =
    data?.data?.roles?.map(({ name }) => ({ label: name, value: name })) ?? [];
  return <ThemedSelect {...props} options={options} />;
};

const SelectItems: React.FC<
  Partial<Async<OptionTypeBase>> & Props<OptionTypeBase, false>
> = ({ variant = 'outlined', additionalStyle, ...props }) => {
...
const SingleValue = (
  props: SingleValueProps<{ label: string; value: string; data: Item }>
) => {
  ...
  return (
    <components.SingleValue {...props}>
      <div>
        ...
        <div>
          Harga jual : {formatToIDR(data?.data?.sell_price ?? 0)} | stock :{' '}
          {data.data?.quantity} | id :{data?.data?.item_id ?? '-'}
        </div>
      </div>
...
const SelectSortBy: React.FC<ThemedSelectProps> = ({
  disableMargin,
  ...props
}) => {
  const styles = {
...
type CurrencyTextFieldProps = Omit<NumericFormatProps, 'onChange'> & {
  prefix?: string;
  ...
  return (
    <>
      ...
      {errors[name] && touched[name] && (
        <span className='text-xs text-red-500'>{errors[name] as string}</span>
      )}
    </>
  );
};
