...
import {
  RegionCitiesRes,
  RegionProvincesRes,
  RegionSubdistrictRes,
  RegionVilageRes,
} from '@/typings/regions';
...
export const useSearchProvince = () => {
  return useMutation<BackendRes<RegionProvincesRes>, void, SearchParam>(
    ['province'],
    async (jsonBody) => {
...
export const useSearchCity = () => {
  return useMutation<BackendRes<RegionCitiesRes>, void, SearchParam>(
    ['cities'],
    async (jsonBody) => {
...
export const useSearchSubdistrict = () => {
  return useMutation<BackendRes<RegionSubdistrictRes>, void, SearchParam>(
    ['subdistricts'],
    async (jsonBody) => {
...
export const useSearchVillage = () => {
  return useMutation<BackendRes<RegionVilageRes>, void, SearchParam>(
    ['village'],
    async (jsonBody) => {
...
export const useSearchRoles = () => {
  return useMutation<BackendRes<RolesResponse>, void, SearchParam>(
    ['roles'],
    async (jsonBody) => {
...
export const useSearchItems = () => {
  ...
  return useMutation<BackendRes<ItemsResponse>, void, SearchParam>(
    ['items'],
    async (jsonBody) => {
      const { data } = await getApiBasedOnRoles(roles, [
        'superadmin',
        'admin',
        'warehouse-admin',
      ]).post('/items', jsonBody);
...
export const useSearchSuppliers = () => {
  ...
  return useMutation<BackendRes<SuppliersResponse>, void, SearchParam>(
    ['suppliers'],
    async (jsonBody) => {
      const { data } = await getApiBasedOnRole(
        dataSelf?.data?.user?.roles[0]?.name || ''
      ).post('/suppliers', jsonBody);
      return data;
    }
  );
};
...
export const useSearchCustomers = () => {
  const { data: dataSelf } = useFetchMyself();
  const roles = dataSelf?.data.user.roles.map(({ name }) => name) ?? [];
  return useMutation<BackendRes<CustomersResponse>, void, SearchParam>(
    ['suppliers'],
    async (jsonBody) => {
      const { data } = await getApiBasedOnRoles(roles, [
        'superadmin',
        'admin',
      ]).post('/customers', jsonBody);
      return data;
    }
  );
};
